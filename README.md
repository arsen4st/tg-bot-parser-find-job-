...
Быстрый асинхронный парсер вакансий из Telegram-каналов. Сам следит за новыми постами, фильтрует по профилям поиска (требования, зарплата, актуальность) и шлёт в личку бота с кнопкой «Перейти» на конкретный пост.

## ⚡ Особенности

- **Максимальная скорость**: asyncio + Telethon в режиме реального времени.
- **Профили поиска**: несколько профилей с разными фильтрами, один активен одновременно.
- **Фильтр по требованиям**: включи/исключи ключевые слова (`+python +удалённо -senior`).
- **Фильтр по зарплате**: задай минимальную сумму.
- **Проверка актуальности**: не показывать вакансии старше N дней.
- **Красивые карточки**: авто-заголовок, зарплата, теги, форматирование.
...
- `/setreq <+слово -слово>` — задать фильтр активного профиля
- `/requirements` — текущие настройки активного профиля
- `/setminsalary 50000` — минимальная зарплата активного профиля
- `/salaryfilter on/off` — включить/выключить фильтр по зарплате
- `/setmaxage 7` — максимальный возраст вакансии в днях
